...
// Build the local logo URL from this instance's configured EPG endpoint.
// This keeps generated playlists portable across CTs and avoids stale hostnames.
...
func GenerateXmlTv(daysAgo int) ([]byte, error) {
	...
	for _, info := range newChanInfo {
		...
		global.DB.Where("comm_name = ?", info.CommName).
			Where("end_time > ?", playableBoundary).
