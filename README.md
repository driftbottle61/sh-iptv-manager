...
最新发行版为 `v1.2.24`。下载 `releases/sh-iptv-spider-installer-1.2.24-linux-amd64.tar.gz` 后执行：

```bash
tar -xzf sh-iptv-spider-installer-1.2.24-linux-amd64.tar.gz
...
```bash
curl -fsSL https://github.com/driftbottle61/sh-iptv-manager/releases/download/v1.2.24/install-oneclick.sh | bash
```
